...
function test(){
    ...
    if(fileLx == "bmp" || fileLx == "png"|| fileLx == "jpeg"|| fileLx == "jpg"|| fileLx == "gif")
    {
    	if(file.size>10*1024*1024){
        	alert("封面太大了！最大可传10M的封面！");
    ...
    {
     alert("只支持图片的上传！格式有：bmp、png、jpeg、jpg、gif");
...
<body>
	...
	<div data-role="header" class="header"></div>
		...
		<div class="form">
		<form class="form-horizontal" id="upload" name="upload" method="post" action="${pageContext.request.contextPath }/picUpload.do" enctype="multipart/form-data">
		
		<div class="file"><div class="file1"><font color=white >封面上传:</font></div>
		...
		<div class="content"><div class="box">
					<input type="file" name="uploadFile" id="uploadFile" class="inputfile inputfile-1" data-multiple-caption="{count} files selected" multiple  onchange="test()"/>
					<label for="uploadFile"><svg xmlns="http://www.w3.org/2000/svg" width="20" height="17" viewBox="0 0 20 17"><path d="M10 0l-5.2 4.9h3.3v5.1h3.8v-5.1h3.3l-5.2-4.9zm9.3 11.5l-3.2-2.1h-2l3.4 2.6h-3.5c-.1 0-.2.1-.2.1l-.8 2.3h-6l-.8-2.2c-.1-.1-.1-.2-.2-.2h-3.6l3.4-2.6h-2l-3.2 2.1c-.4.3-.7 1-.6 1.5l.6 3.1c.1.5.7.9 1.2.9h16.3c.6 0 1.1-.4 1.3-.9l.6-3.1c.1-.5-.2-1.2-.7-1.5z"/></svg> <span>选取封面&hellip;</span></label>
				</div>
		...
		<%
			String url=request.getParameter("videoSRC");

		%>
		<input type="hidden"  name="videoSRC" value=<%=url%>>
		...
		<div class="description2"><textarea class="descript" id="descript"name="descript" style="resize:none;height:100px;" placeholder="有趣的动态描述，会增加被小编捕捉为热门动态的机会哟 (=ﾟωﾟ)ﾉ" maxlength="233"></textarea></div></div>
