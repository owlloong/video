...
public class db {
	...
	// private String sConnStr = "jdbc:microsoft:sqlserver://localhost:1433;databasename=ssmpajxglxthsg4461AEY3";

	private String dbDriver="com.mysql.jdbc.Driver";
	private String sConnStr = "jdbc:mysql://localhost:3306/cilicili?useUnicode=true&amp;amp;amp;amp;amp;characterEncoding=utf-8";

	// private String dbDriver="net.sourceforge.jtds.jdbc.Driver";
	// private String sConnStr = "jdbc:jtds:sqlserver://127.0.0.1:1433;databaseName=ssmpajxglxthsg4461AEY3";
	...
	public String hsggetoption2(String ntable,String nzd){
		StringBuffer imgStr = new StringBuffer();
		imgStr.append("");
		imgStr.append("<option value=''>ÇëÑ¡Ôñ</option>\n");
	...
	public static int getBetweenDayNumber(String dateA, String dateB) {
		long dayNumber = 0;
		//1Ð¡Ê±=60·ÖÖÓ=3600Ãë=3600000
		long mins = 60L * 1000L;
		//long day= 24L * 60L * 60L * 1000L;¼ÆËãÌìÊýÖ®²î
	...
	public List getShouyebht(){
		String sql = "select id,biaoti,shouyetupian from xinwentongzhi where leibie='Í¨Öª¹«¸æ' order by id desc";
		list = new ArrayList();
		try{
			connect=DriverManager.getConnection(sConnStr,nsa,nmm);
			Statement stmt=connect.createStatement();
	...
	public String getxwlb(){
		...
		String sql="";
		sql="select distinct(leibie) as ss from xinwentongzhi where leibie<>'±ä»ÃÍ¼'";
		System.out.print(sql);
		try{
			connect=DriverManager.getConnection(sConnStr,nsa,nmm);
			Statement stmt=connect.createStatement();
	...
	public String getddnr(String nuser){
		...
		String sql = "select shangpinmingcheng,goumaishuliang from goumaijilu where goumairen = '"+nuser+"' and issh='·ñ' ";
		System.out.print(sql);
		...
		try{
			...
			while(rs.next()){
				...
				ddnr=ddnr+"ÉÌÆ·Ãû³Æ£º"+rs.getString(1)+"£¬¹ºÂòÊýÁ¿£º"+rs.getString(2)+"£»\r\n";
			}
			return ddnr;
		}catch(Exception e){
			e.printStackTrace();
